...
public class VillaController : ControllerBase
{
    ...
    [ProducesResponseType(typeof(ApiResponse<object>), StatusCodes.Status400BadRequest)]
    [ProducesResponseType(typeof(ApiResponse<object>), StatusCodes.Status409Conflict)]
    public async Task<ActionResult<ApiResponse<VillaDTO>>> UpdateVilla(int id, VillaUpdateDTO model)
    {
        if (model is null)
            return BadRequest(ApiResponse<object>.BadRequest("Invalid request data"));

        if (id != model.Id)
            return BadRequest(ApiResponse<object>.BadRequest("Villa Id does not match the model ID in request body"));
